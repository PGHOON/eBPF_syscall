...
int main(int argc, char **argv)
{
	...
	skel = monitor3_bpf__open_opts(&opts);

	err = monitor3_bpf__load(skel);
	
	if (err) {
		printf("Failed to load BPF object\n");
		monitor3_bpf__destroy(skel);
		return 1;
	}

	// Attach the progams to the events
	err = monitor3_bpf__attach(skel);
	if (err) {
